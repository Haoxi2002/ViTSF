...
from layers.Transformer_EncDec import Encoder, EncoderLayer, ConvLayer, EncoderStack, Decoder, DecoderLayer
from layers.SelfAttention_Family import FullAttention, ProbAttention, AttentionLayer_KAE
...
class AggreOutput(nn.Module):
    def __init__(self, kernel_size=-1, in_features=120, hidden_size=256, out_features=60):
        ...
        if kernel_size < 0:
            ...
            self.out_layer = nn.Conv1d(hidden_size, out_features, kernel_size=kernel_size,
...
class Informer(nn.Module):
    def __init__(self, enc_in, dec_in, c_out, seq_len, label_len, out_len,
        ...
        self.encoder = Encoder(
            [
                EncoderLayer(
                    AttentionLayer_KAE(Attn(False, factor, attention_dropout=dropout, output_attention=output_attention),
        ...
        self.decoder = Decoder(
            [
                DecoderLayer(
                    AttentionLayer_KAE(Attn(True, factor, attention_dropout=dropout, output_attention=False),
                                   d_model, n_heads, mix=mix, kernel_size=qvk_kernel_size),
                    AttentionLayer_KAE(FullAttention(False, factor, attention_dropout=dropout, output_attention=False),
                                   d_model, n_heads, mix=False, kernel_size=qvk_kernel_size),
                    d_model,
                    d_ff,
                    dropout=dropout,
...
class Model(nn.Module):
    def __init__(self, configs, ts_factor=5, ts_d_model=512, ts_n_heads=8, ts_e_layers=3, ts_d_layers=2, ts_d_ff=512,
                 ts_dropout=0.0, ts_attn='prob', ts_embed='fixed', ts_activation='gelu',
                 ts_distil=True, ts_mix=True, ts_qvk_kernel_size=5, event_factor=3, event_d_model=384, event_n_heads=8,
                 event_e_layers=2, event_d_layers=1, event_d_ff=384,
        ...
        ts_enc_in = configs.enc_in
        ts_dec_in = configs.enc_in
        ts_c_out = configs.enc_in
        ts_seq_len = configs.seq_len
        ts_label_len = configs.label_len
        ts_out_len = configs.pred_len
        event_seq_len = configs.seq_len
        event_label_len = configs.label_len
        event_out_len = configs.pred_len
        self.ts_informer = Informer(enc_in=ts_enc_in, dec_in=ts_dec_in, c_out=ts_c_out, seq_len=ts_seq_len,
                                    label_len=ts_label_len,
                                    ...
                                    attn=ts_attn, embed=ts_embed, freq=freq, activation=ts_activation,
        ...
        self.event_informer = Informer(enc_in=ts_out_len, dec_in=ts_out_len, c_out=ts_out_len, seq_len=event_seq_len,
                                       label_len=event_label_len, out_len=event_out_len, factor=event_factor,
                                       d_model=event_d_model,
                                       n_heads=event_n_heads, e_layers=event_e_layers, d_layers=event_d_layers,
                                       d_ff=event_d_ff,
                                       ...
                                       distil=event_distil, mix=event_mix, qvk_kernel_size=event_qvk_kernel_size,
                                       device=device)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        if type <= 1:
            return self.ts_informer.forward(x_enc=x_enc, x_mark_enc=x_mark_enc, x_dec=x_dec,
                                            x_mark_dec=x_mark_dec, enc_self_mask=enc_self_mask,
...
class CSEAInformer(nn.Module):
    def __init__(self, ts_enc_in, ts_dec_in, ts_c_out, ts_seq_len, ts_label_len, ts_out_len,
        ...
        if out_kernel_size < 0:
            self.final_output = AggreOutput(kernel_size=out_kernel_size, hidden_size=out_hidden_size,
                                            in_features=ts_out_len * 2, out_features=ts_out_len)
        else:
            self.final_output = AggreOutput(kernel_size=out_kernel_size, hidden_size=out_hidden_size,
                                            in_features=event_out_len * 2, out_features=event_out_len)
